...
//		earthquakesURL = "test2.atom";
	...
	private void addKey() {
		...
		// Low Magnitude
		fill(color(95, 117, 142));
		ellipse(63, 240, 15, 15);

		// Average Magnitude
		fill(color(230, 242, 92));
		ellipse(63, 270, 15, 15);

		// High Magnitude
		fill(color(219, 22, 47));
	...
	private boolean isLand(PointFeature earthquake) {
		...
		for (Marker m : countryMarkers) {
			...
			boolean b = isInCountry(earthquake, m);
			if (b) {
				return b;
			}
		}
	...
	private void printQuakes() 
	{
		...
		for (Marker m : countryMarkers) {
			int count = 0;
			String country = (String) m.getProperty("name");
			for (Marker q : quakeMarkers) {
				EarthquakeMarker em = (EarthquakeMarker) q;
				if (em.isOnLand()) {
					if (((String)q.getProperty("country")).equals(country)) {
						count++;
					}
				}
			}
		...
		System.out.println("Ocean Quakes : " + (quakeMarkers.size() - totalLand));
